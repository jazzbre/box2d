...
void b2ParticleSystem::Draw(b2Draw* draw) const {
	...
	const b2ParticleColor* colorBuffer = m_colorBuffer.data ? GetColorBuffer() : nullptr;
	for (int32 i = 0; i < particleCount; ++i) {
		draw->DrawCircle(positionBuffer[i], radius, colorBuffer != nullptr ? colorBuffer[i].GetColor() : b2Color(1.0f, 1.0f, 1.0f, 1.0f));
	}
}
								...
								b2Fixture* fixture, int32 childIndex, int32 a)
		{
			if (ShouldCollide(fixture, a)) {
				...
				if (m_system->m_iterationIndex == 0)
				{
					...
					b2Vec2 p1 = b2MulT(body->GetPreviousTransform(), ap);
					if (fixture->GetShape()->GetType() == b2Shape::e_circle)
					{
						...
						// circle
						p1 = b2Mul(body->GetPreviousTransform().q, p1);
						// Subtract rotation of the current frame
						p1 = b2MulT(body->GetTransform().q, p1);
						// Return to local space
						p1 += body->GetLocalCenter();
					}
					// Return to global space and apply rotation of current frame
					input.p1 = b2Mul(body->GetTransform(), p1);
				}
				else
				{
					input.p1 = ap;
...
int64_t b2ParticleSystem::LifetimeToExpirationTime(const float lifetime) const
{
	return m_timeElapsed + (int64_t)((lifetime / m_def.lifetimeGranularity) *
								   (float)(1LL << 32));
}
								  ...
								  const b2Vec2& force)
{
	...
	b2Vec2 distributedForce = force;
	distributedForce *= (1.0f / (float)(lastIndex - firstIndex));
										  ...
										  const b2Vec2& impulse)
{
	...
	b2Vec2 velocityDelta = impulse;
	velocityDelta *= 1.0f / totalMass;
	for (int32 i = firstIndex; i < lastIndex; i++)
	{
